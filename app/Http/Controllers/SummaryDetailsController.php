...
class SummaryDetailsController extends Controller {
    ...
    public function getSummaryDetails(Request $request) {
        ...
        $query -> leftJoin('tblSummary', 'tblSummary_Details.SummaryId', '=', 'tblSummary.SummaryId')
            ...
            ->select('tblSummary_Details.Summary_DetailsId','tblSummary.Summary', 'tblEra.Era', 'tblCategory.Category', 'tblSubCategory.Subcategory',
                'tblSource.SourceName', 'tblSource_Type.Source_Type','tblSource_Quoted.BeginChptrSectionMinute', 'tblSource_Quoted.BeginVersePageSecond',
                'tblSource_Quoted.EndChptrSectionMinute', 'tblSource_Quoted.EndVersePageSecond', 'tblSource_Quoted.Source_Explanation',
                'tblQuotation.Quotation', 'tblStatusType.StatusType', 'tblSource_Type.Source_Type_Abbreviation');
    ...
    public function postUpdateSummaryDetails(Request $request) {
        ...
        try{
            ...
            $sourceOuotedObj = SourceQuoted::find($summaryDetails->Source_QuotedId);
            $sourceOuotedObj->BeginChptrSectionMinute = $beginChptrSectionMinute;
            $sourceOuotedObj->BeginVersePageSecond = $beginVersePageSecond;
            $sourceOuotedObj->EndChptrSectionMinute = $endChptrSectionMinute;
            $sourceOuotedObj->EndVersePageSecond = $endVersePageSecond;
            $sourceOuotedObj->Source_Explanation = $source_Explanation;
            $sourceOuotedObj->save();

            //update source name;
            $sourceObj = Source::find($sourceOuotedObj->SourceId);
...
//            ->where('SourceId', $request->SourceId )->get();
    ...
    public function getAddSummaryDetails(Request $request) {
        ...
        try {
            $summaryDetais = new SummaryDetails();
            $summaryDetais->SummaryId = $summaryId;
            $summaryDetais->QuotationId = $quotationId;
            $summaryDetais->EraId = $eraId;
            $summaryDetais->SubcategoryId = $subcategoryId;
            $summaryDetais->Source_QuotedId = $sourceQuotedId;
            $summaryDetais->StatusTypeId = 6;
//        $summaryDetais->LocationId = $locationId;
//        $summaryDetais->SacrificeId = $sacrificeId;
    ...
    public function getAddEra(Request $request) {
        ...
        $Era = $request->Era;
        $BeginYearId = $request->BeginYearId;
        $EndYearId = $request->EndYearId;

        if($Era == '' || $BeginYearId == '' || $EndYearId == '') {
        ...
        try{
            ...
            DB::table('tblEra_BeginEnd')->insert(
                ['EraId' => $era->EraId, 'BeginYearId' => $BeginYearId, 'EndYearId' => $EndYearId]
            );
